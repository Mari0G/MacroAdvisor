...
| 4. Gemini provider adapter | In review | `codex/slice4-gemini` | `test/src/features/meal_capture/infrastructure/gemini_nutrition_analysis_provider_test.dart` and sanitized fixtures | Gemini adapter, categorized failures, connection check, validation, redaction, and timeout evidence pass locally; not merged. |
